...
const createRailsFiles = (options) => {
    ...
    if (options.tailwind) {
        ...
        writeFile(path.join(process.cwd(), 'config', 'tailwind.config.js'), TEMPLATES.tailwind(options));
    }
    ...
    createFile(homePath, 'index.js', TEMPLATES.index(options));
    createFile(componentsPath, 'counter.js', TEMPLATES.counter(options));
}

const createRailsProject = (options) => {
