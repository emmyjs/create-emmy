...
<html>
  ...
  <body class="antialiased h-screen w-screen flex flex-col justify-center items-center bg-slate-900">
    <main class="flex flex-col justify-center items-center space-y-3 text-center w-full h-full">
      ...
      `<script type='module' src='../node_modules/emmy-dom/dist/index.js'></script>
...
`,
  counter: (options) =>
`import { load } from "${ options.prerender ? 'emmy-dom/dist/server.js' : 'emmy-dom' }";
...
export function app() {
  this.behave('div');
  this.className = 'flex flex-col justify-center items-center space-y-3 text-center w-full h-full text-white';
